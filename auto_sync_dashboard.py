...
def http_get_json(
    ...
    *,
    ...
    if response.status_code >= 400:
        print("Elsevier/OpenAlex HTTP error")
...
def scopus_entry_to_record(entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "Journal": (
            ...
            or entry.get("prism:aggregationType")
            or "Unknown"
        ).strip(),
        "Citations": cited_by,
...
def fetch_scopus_records(api_key: str, author_id: str, verbose: bool) -> List[Dict[str, Any]]:
    ...
    out.sort(
        key=lambda r: (
            ud.parse_int(r.get("Year")),
            ud.parse_int(r.get("Citations")),
        ),
        reverse=True,
    )
...
def update_scholar_block_from_openalex(payload: Dict[str, Any], author: Dict[str, Any]) -> None:
    ...
    scholar.update(
        {
            "citations_total": ud.parse_int(
                author.get("cited_by_count"),
                scholar.get("citations_total", 0),
            ),
            "h_index": ud.parse_int(
                summary.get("h_index"),
                scholar.get("h_index", 0),
            ),
            "i10_index": ud.parse_int(
                summary.get("i10_index"),
                scholar.get("i10_index", 0),
            ),
        }
    )
...
def main() -> int:
    ...
    author_id = (
        args.author_id
        or os.getenv("SCOPUS_AUTHOR_ID")
        or SCOPUS_AUTHOR_ID_DEFAULT
    ).strip()

    orcid = (
        args.orcid
        or os.getenv("ORCID_ID")
        or ORCID_ID_DEFAULT
    ).strip()
    ...
    payload["meta"].update(
        {
            "full_name": payload.get("meta", {}).get(
                "full_name",
    ...
    auto_push = (
        args.git_push
        or os.getenv("GIT_AUTO_PUSH", "0").strip().lower()
        in {"1", "true", "yes"}
    )
